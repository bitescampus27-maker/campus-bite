...
const Kitchen = () => {
  ...
  const loadOrders = async () => {
    try {
      const onlineRes = await axios.get(
        "https://campus-bite-backend.onrender.com/api/order/kitchen"
      );
      const posRes = await axios.get(
        "https://campus-bite-backend.onrender.com/api/pos/orders"
      );
  ...
  const markPrepared = async (order) => {
    try {
      if (order.isPOS) {
        await axios.post("https://campus-bite-backend.onrender.com/api/pos/update-status", {
          orderId: order._id,
          status: "prepared",
        });
      } else {
        await axios.post("https://campus-bite-backend.onrender.com/api/order/prepared", {
          orderId: order._id,
          status: "prepared",
        });
      }
